...
function PromptSection() {
  ...
  const hasGeminiKey = !!localStorage.getItem('sitebuilder-gemini-key')
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[30%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] bg-brand/[0.07] rounded-full blur-[150px]" />
      </div>
      ...
      <div className="relative flex flex-col items-center pt-16 pb-6 px-6">
        ...
        <div className={`w-full max-w-[680px] rounded-2xl p-px transition-all duration-300 animate-scale-in stagger-3 ${
          isFocused
            ? 'bg-gradient-to-b from-brand/40 via-brand/20 to-brand/5 shadow-[0_0_80px_rgba(34,197,94,0.15)]'
            : 'bg-gradient-to-b from-border-hover via-border-default to-border-subtle shadow-[0_0_60px_rgba(34,197,94,0.06)] hover:from-brand/25 hover:via-brand/10 hover:to-brand/5 hover:shadow-[0_0_80px_rgba(34,197,94,0.1)]'
        }`}>
            ...
            <div className="flex items-center justify-between px-4 pb-3 pt-1">
              ...
              <div className="flex items-center gap-2 shrink-0 ml-3">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-brand text-black text-[13px] font-semibold hover:bg-brand-dim active:scale-[0.97] transition-all disabled:opacity-20 disabled:cursor-not-allowed inline-flex items-center gap-2 shadow-[0_0_20px_rgba(34,197,94,0.3)] hover:shadow-[0_0_30px_rgba(34,197,94,0.4)]"
                >
                  <Sparkles size={14} />
                  Generate
                </button>
        ...
        <div className="mt-3 flex flex-col items-center gap-1.5">
          ...
          {!hasGeminiKey && (
            <p className="text-text-3 text-[10.5px]">
              Using template mode.{' '}
              <NavLink to="/settings" className="text-brand hover:text-brand-dim transition-colors">
      ...
      <div className="h-32 bg-bg-2 relative overflow-hidden">
        ...
        <div className="absolute top-2 right-2 z-10 flex gap-1">
          <button
            ...
            className="p-1.5 rounded-md border bg-bg-0/80 border-border-default text-text-3 opacity-0 group-hover:opacity-100 hover:text-brand hover:border-brand/30 transition-all"
          >
            <Copy size={12} />
          </button>
          <button
      ...
      <div className="px-4 py-3">
        {editing ? (
          <input
            ...
            className="text-[13px] font-semibold mb-1 bg-transparent border-b border-brand outline-none w-full"
          />
        ) : (
          <div
          ...
          <span className="flex items-center gap-1.5">
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                project.status === 'published' ? 'bg-brand' : 'bg-status-yellow'
              }`}
            />
          ...
          <div className="px-4 md:px-12 pt-5 flex flex-col sm:flex-row gap-2 items-start sm:items-center sm:justify-between">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-7 pr-3 py-1.5 rounded-md border border-border-default bg-bg-2 text-text-0 text-[12px] w-44 outline-none focus:border-brand placeholder:text-text-3"
              />
            </div>
          </div>
          ...
          {filtered.length > 0 ? (
            ...
            <div className="px-4 md:px-12 pt-8 pb-12 flex flex-col items-center text-center">
              ...
              <button
                ...
                className="mt-2 text-brand text-[12px] hover:text-brand-dim transition-colors"
              >
                Clear filters
              </button>
            </div>
