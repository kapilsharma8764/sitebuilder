...
function ControlledInput({ settingsKey, placeholder, settings }: { settingsKey: string; placeholder?: string; settings: ReturnType<typeof useSettingsState> }) {
  return (
    <input
      ...
      className="w-full px-3 py-2 rounded-lg border border-border-default bg-bg-2 text-text-0 text-[13px] outline-none focus:border-brand placeholder:text-text-3 transition-colors"
    />
  )
}
...
function ControlledTextarea({ settingsKey, rows = 3, settings }: { settingsKey: string; rows?: number; settings: ReturnType<typeof useSettingsState> }) {
  return (
    <textarea
      ...
      rows={rows}
      onChange={(e) => settings.update(settingsKey, e.target.value)}
      className="w-full px-3 py-2 rounded-lg border border-border-default bg-bg-2 text-text-0 text-[13px] outline-none focus:border-brand resize-y transition-colors"
    />
  )
}
    ...
    <div>
      ...
      <FieldGroup label="Language">
        <select
          ...
          className="w-full px-3 py-2 rounded-lg border border-border-default bg-bg-2 text-text-0 text-[13px] outline-none focus:border-brand cursor-pointer"
        >
...
function SeoPanel({ settings }: { settings: ReturnType<typeof useSettingsState> }) {
  const title = settings.data.seoTitle || 'My Website - Build with SiteBuilder'
...
function ApiPanel({ settings }: { settings: ReturnType<typeof useSettingsState> }) {
  const [geminiKey, setGeminiKey] = useState(localStorage.getItem('sitebuilder-gemini-key') || '')
  ...
  function handleKeyChange(value: string) {
    setGeminiKey(value)
    if (value) {
      localStorage.setItem('sitebuilder-gemini-key', value)
    } else {
      localStorage.removeItem('sitebuilder-gemini-key')
    }
  }
    ...
    <div>
      ...
      <FieldGroup label="Deploy Access Key">
        <ControlledInput
          ...
          placeholder="Must match SITEBUILDER_DEPLOY_KEY on server"
          settings={settings}
        />
      ...
      <FieldGroup label="Gemini API Key">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-lg border border-border-default bg-bg-2 text-text-0 text-[13px] outline-none focus:border-brand placeholder:text-text-3 transition-colors font-mono"
          />
          <button
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg bg-brand/10 text-brand text-[12px] font-medium hover:bg-brand/20 transition-colors shrink-0 disabled:opacity-40 disabled:cursor-not-allowed"
          >
