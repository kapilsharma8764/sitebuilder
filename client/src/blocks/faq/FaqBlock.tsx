...
const defaultFaqs: FaqItem[] = [
  { question: 'What is SiteBuilder?', answer: 'SiteBuilder is a visual website builder that uses structured JSON config as the source of truth. Both humans and AI agents can edit the same config to build beautiful websites.' },
  ...
  { question: 'Can I use my own components?', answer: 'Yes! SiteBuilder supports custom components. You can build your own blocks following our component schema and register them in the block registry.' },
  { question: 'Is it free to use?', answer: 'SiteBuilder offers a free tier for personal projects with up to 5 blocks. Pro and Team plans unlock unlimited blocks, custom domains, and priority support.' },
]
...
function AccordionItem({ item, isOpen, onToggle }: { item: FaqItem; isOpen: boolean; onToggle: () => void }) {
  return (
    <div className="border-b border-border-subtle">
      ...
      >
        <span className="text-[13.5px] font-medium text-text-0 group-hover:text-brand transition-colors">
          {item.question}
        </span>
        <ChevronDown
          size={16}
