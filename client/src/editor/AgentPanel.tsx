...
function TypingIndicator() {
  return (
    <div className="self-start flex gap-1 px-4 py-3 bg-brand-glow rounded-xl rounded-bl-sm border border-brand/10">
      {[0, 1, 2].map((i) => (
        <span
          key={i}
...
export function AgentPanel() {
  ...
  function handleSend() {
    ...
    const userMsg: ChatMessage = { id: newId('msg'), role: 'user', text }
    ...
    setTimeout(() => {
      ...
      if ('action' in response) {
        const agentMsg: ChatMessage = { id: newId('msg'), role: 'agent', text: response.message }
          ...
          <div
            ...
            className={`max-w-[94%] px-3 py-2.5 rounded-xl text-[12.5px] leading-relaxed ${
              msg.role === 'user'
                ...
                : 'self-start bg-brand-glow text-text-0 rounded-bl-sm border border-brand/10'
            }`}
          >
            {msg.text}
            ...
            {msg.patch && (
              <div className="bg-bg-2 border border-border-default rounded-md p-2 mt-2 font-mono text-[10.5px] leading-relaxed">
                <div className="flex items-center justify-between mb-1">
                  ...
                  {!msg.applied && (
                    <div className="flex gap-1">
                      <button
                        ...
                        className="px-1.5 py-0.5 rounded text-[9px] bg-brand/20 text-brand hover:bg-brand/30 transition-colors flex items-center gap-0.5"
                      >
                        <Check size={9} /> Apply
                      </button>
                      <button
                  ...
                  {msg.applied && (
                    <span className="text-[9px] text-brand font-medium flex items-center gap-0.5">
                ...
                {msg.patch.added?.map((line, i) => (
                  <div key={`a-${i}`} className="text-brand">
                    + {line}
                  </div>
                ))}
              </div>
      ...
      <div className="p-2.5 border-t border-border-default">
        <div className="flex gap-1.5">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-lg border border-border-default bg-bg-2 text-text-0 text-[13px] outline-none focus:border-brand placeholder:text-text-3"
          />
          <button
            onClick={handleSend}
            className="w-9 h-9 rounded-lg bg-brand flex items-center justify-center text-black shrink-0 hover:bg-brand-dim transition-colors"
        ...
        <div className="flex gap-1 mt-1.5 flex-wrap">
          {['Change the headline', 'Add a pricing section', 'Make the hero split', 'Switch to midnight theme'].map((hint) => (
            <span
              ...
              className="px-2 py-0.5 rounded-full text-[10.5px] text-text-2 border border-border-default bg-bg-2 cursor-pointer hover:border-brand hover:text-brand hover:bg-brand-glow transition-all"
            >
              {hint}
            </span>
          ))}
